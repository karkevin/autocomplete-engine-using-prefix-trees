"""CSC148 Assignment 2: Autocompleter classes

=== CSC148 Fall 2018 ===
Department of Computer Science,
University of Toronto

=== Module Description ===
This file contains the design of a public interface (Autocompleter) and two
implementation of this interface, SimplePrefixTree and CompressedPrefixTree.
You'll complete both of these subclasses over the course of this assignment.

As usual, be sure not to change any parts of the given *public interface* in the
starter code---and this includes the instance attributes, which we will be
testing directly! You may, however, add new private attributes, methods, and
top-level functions to this file.
"""
from __future__ import annotations
from typing import Any, List, Optional, Tuple


################################################################################
# The Autocompleter ADT
################################################################################
class Autocompleter:
    """An abstract class representing the Autocompleter Abstract Data Type.
    """
    def __len__(self) -> int:
        """Return the number of values stored in this Autocompleter."""
        raise NotImplementedError

    def insert(self, value: Any, weight: float, prefix: List) -> None:
        """Insert the given value into this Autocompleter.

        The value is inserted with the given weight, and is associated with
        the prefix sequence <prefix>.

        If the value has already been inserted into this prefix tree
        (compare values using ==), then the given weight should be *added* to
        the existing weight of this value.

        Preconditions:
            weight > 0
            The given value is either:
                1) not in this Autocompleter
                2) was previously inserted with the SAME prefix sequence
        """
        raise NotImplementedError

    def autocomplete(self, prefix: List,
                     limit: Optional[int] = None) -> List[Tuple[Any, float]]:
        """Return up to <limit> matches for the given prefix.

        The return value is a list of tuples (value, weight), and must be
        ordered in non-increasing weight. (You can decide how to break ties.)

        If limit is None, return *every* match for the given prefix.

        Precondition: limit is None or limit > 0.
        """
        raise NotImplementedError

    def remove(self, prefix: List) -> None:
        """Remove all values that match the given prefix.
        """
        raise NotImplementedError


################################################################################
# SimplePrefixTree (Tasks 1-3)
################################################################################
class SimplePrefixTree(Autocompleter):
    """A simple prefix tree.

    This class follows the implementation described on the assignment handout.
    Note that we've made the attributes public because we will be accessing them
    directly for testing purposes.

    === Attributes ===
    value:
        The value stored at the root of this prefix tree, or [] if this
        prefix tree is empty.
    weight:
        The weight of this prefix tree. If this tree is a leaf, this attribute
        stores the weight of the value stored in the leaf. If this tree is
        not a leaf and non-empty, this attribute stores the *aggregate weight*
        of the leaf weights in this tree.
    subtrees:
        A list of subtrees of this prefix tree.
    _weight_type:
        The method in which the weight is calculated; either "sum" or "average."
    _weight_data:
        A list storing the total (summed) weight of the tree and the number of
        leaves in the tree.


    === Representation invariants ===
    - self.weight >= 0

    - (EMPTY TREE):
        If self.weight == 0, then self.value == [] and self.subtrees == [].
        This represents an empty simple prefix tree.
    - (LEAF):
        If self.subtrees == [] and self.weight > 0, this tree is a leaf.
        (self.value is a value that was inserted into this tree.)
    - (NON-EMPTY, NON-LEAF):
        If len(self.subtrees) > 0, then self.value is a list (*common prefix*),
        and self.weight > 0 (*aggregate weight*).

    - ("prefixes grow by 1")
      If len(self.subtrees) > 0, and subtree in self.subtrees, and subtree
      is non-empty and not a leaf, then

          subtree.value == self.value + [x], for some element x

    - self.subtrees does not contain any empty prefix trees.
    - self.subtrees is *sorted* in non-increasing order of their weights.
      (You can break ties any way you like.)
      Note that this applies to both leaves and non-leaf subtrees:
      both can appear in the same self.subtrees list, and both have a `weight`
      attribute.
    """
    value: Any
    weight: float
    subtrees: List[SimplePrefixTree]
    _weight_type: str
    _weight_data: [float, int]

    def __init__(self, weight_type: str) -> None:
        """Initialize an empty simple prefix tree.

        Precondition: weight_type == 'sum' or weight_type == 'average'.

        The given <weight_type> value specifies how the aggregate weight
        of non-leaf trees should be calculated (see the assignment handout
        for details).
        """
        self.value = []
        self.weight = 0.0
        self.subtrees = []
        self._weight_type = weight_type
        self._weight_data = [0.0, 0]

    def is_empty(self) -> bool:
        """Return whether this simple prefix tree is empty."""
        return self.weight == 0.0

    def is_leaf(self) -> bool:
        """Return whether this simple prefix tree is a leaf."""
        return self.weight > 0 and self.subtrees == []

    def __str__(self) -> str:
        """Return a string representation of this tree.

        You may find this method helpful for debugging.
        """
        return self._str_indented()

    def _str_indented(self, depth: int = 0) -> str:
        """Return an indented string representation of this tree.

        The indentation level is specified by the <depth> parameter.
        """
        if self.is_empty():
            return ''
        else:
            s = '  ' * depth + f'{self.value} ({self.weight})\n'
            for subtree in self.subtrees:
                s += subtree._str_indented(depth + 1)
            return s

    def __len__(self) -> int:
        """Return the number of values stored in this Autocompleter."""

        return self._weight_data[1]

    def insert(self, value: Any, weight: float, prefix: List) -> None:
        """Insert the given value into this Autocompleter.

        The value is inserted with the given weight, and is associated with
        the prefix sequence <prefix>.

        If the value has already been inserted into this prefix tree
        (compare values using ==), then the given weight should be *added* to
        the existing weight of this value.

        Preconditions:
            weight > 0
            The given value is either:
                1) not in this Autocompleter
                2) was previously inserted with the SAME prefix sequence
        """
        if self.value == prefix:
            if any(sub.is_leaf() for sub in self.subtrees):
                inserted = False
                for subtree in self.subtrees:
                    if subtree.value == value and subtree.is_leaf():
                        subtree._weight_data[0] += weight
                        subtree.weight += weight
                        self._weight_data[0] += weight
                        self._adjust_weight()
                        self._sort_subtrees()
                        inserted = True
                        break
                if not inserted:
                    self._insert_leaf(value, weight)
            else:
                self._insert_leaf(value, weight)

        elif any(sub.value == prefix[:len(sub.value)]
                 and not sub.is_leaf() for sub in self.subtrees):
            i = 0
            while self.subtrees[i].value != prefix[:len(
                    self.subtrees[i].value)] or self.subtrees[i].is_leaf():
                i += 1
            subtree = self.subtrees[i]
            old_sub_length = len(subtree)
            subtree.insert(value, weight, prefix)
            new_sub_length = len(subtree)
            self._weight_data[0] += weight
            self._weight_data[1] += new_sub_length - old_sub_length
            self._adjust_weight()
            self._sort_subtrees()
        else:
            subtree = SimplePrefixTree(self._weight_type)
            subtree.value = prefix[:len(self.value) + 1]
            subtree.insert(value, weight, prefix)
            self._weight_data[0] += weight
            self._weight_data[1] += 1
            self._adjust_weight()
            self._add_to_subtrees(subtree)

    def _insert_leaf(self, value: Any, weight: float) -> None:
        """ Insert a leaf into an internal node that matches it's prefix.
        """
        subtree = SimplePrefixTree(self._weight_type)
        subtree.value = value
        subtree.weight = weight
        subtree._weight_data = [float(weight), 1]
        self._weight_data[0] += weight
        self._weight_data[1] += 1
        self._adjust_weight()
        self._add_to_subtrees(subtree)

    def _add_to_subtrees(self, subtree: SimplePrefixTree) -> None:
        """ A helper for the insert method that adds a subtree to self.subtrees.
        """
        if self.is_empty():
            self.subtrees = [subtree]
        else:
            inserted = False
            for i in range(len(self.subtrees)):
                if subtree.weight > self.subtrees[i].weight:
                    self.subtrees.insert(i, subtree)
                    inserted = True
                    break
            if not inserted:
                self.subtrees.append(subtree)

    def _adjust_weight(self) -> None:
        """ A helper for the insert method that adjusts the weight based on a
        given weight type."""
        if self._weight_type == "sum":
            self.weight = self._weight_data[0]
        elif self._weight_type == "average":
            self.weight = self._weight_data[0] / float(self._weight_data[1]) \
                if self._weight_data[0] != 0 else 0
        else:
            raise ValueError

    def _sort_subtrees(self) -> None:
        """ A helper for the insert method that sorts the subtrees in
        self.subtrees. """
        storage = [sub for sub in self.subtrees]
        self.subtrees = []
        for subtree in storage:
            self._add_to_subtrees(subtree)

    def autocomplete(self, prefix: List,
                     limit: Optional[int] = None) -> List[Tuple[Any, float]]:
        """Return up to <limit> matches for the given prefix.

        The return value is a list of tuples (value, weight), and must be
        ordered in non-increasing weight. (You can decide how to break ties.)

        If limit is None, return *every* match for the given prefix.

        Precondition: limit is None or limit > 0.
        """
        if self.is_empty():
            return []
        elif prefix in ([], self.value):
            return self._autocomplete_helper(limit)
        else:
            for subtree in self.subtrees:
                if subtree.value == prefix[:len(subtree.value)]:
                    return subtree.autocomplete(prefix, limit)
            return []

    def _autocomplete_helper(self, limit: Optional[int] = None) \
            -> List[Tuple[Any, float]]:
        """ A helper for the autocomplete method that returns an autocompleted
        list with a given limit.
        """
        if self.is_leaf():
            return [(self.value, self.weight)]
        else:
            accumulator = []
            for subtree in self.subtrees:
                if len(accumulator) == limit:
                    break
                ac_subtree = subtree._autocomplete_helper(limit)
                for value in ac_subtree:
                    if limit is None or len(accumulator) < limit:
                        _acc_insert(accumulator, value)
                    else:
                        break
            return accumulator

    def remove(self, prefix: List) -> None:
        """Remove all values that match the given prefix.
        """
        if not prefix:
            self.subtrees = []
            self.weight = 0.0
            self._weight_data = [0, 0]
        else:
            self._remove_helper(prefix)
            self._sort_subtrees()

    def _remove_helper(self, prefix: List) -> Optional[(float, int)]:
        """ A helper for the remove method that returns an optional tuple that
        keeps track of change in total weight and change in total number of
        leaves.
        """
        if self.value == prefix:
            self.weight = 0.0
            self.subtrees = []
            return self._weight_data[0], self._weight_data[1]
        else:
            total_big = 0
            count_big = 0
            for subtree in reversed(self.subtrees):
                if subtree.value == prefix[:len(subtree.value)]:
                    total, count = subtree._remove_helper(prefix)
                    self._weight_data[0] -= total
                    self._weight_data[1] -= count
                    total_big += total
                    count_big += count
                    if subtree.is_empty():
                        self.subtrees.remove(subtree)
                    self._adjust_weight()
            self._sort_subtrees()
            return total_big, count_big

################################################################################
# CompressedPrefixTree (Task 6)
################################################################################
class CompressedPrefixTree(Autocompleter):
    """A compressed prefix tree implementation.

    While this class has the same public interface as SimplePrefixTree,
    (including the initializer!) this version follows the implementation
    described on Task 6 of the assignment handout, which reduces the number of
    tree objects used to store values in the tree.

    === Attributes ===
    value:
        The value stored at the root of this prefix tree, or [] if this
        prefix tree is empty.
    weight:
        The weight of this prefix tree. If this tree is a leaf, this attribute
        stores the weight of the value stored in the leaf. If this tree is
        not a leaf and non-empty, this attribute stores the *aggregate weight*
        of the leaf weights in this tree.
    subtrees:
        A list of subtrees of this prefix tree.
    _weight_type:
        The method in which the weight is calculated; either "sum" or "average."
    _weight_data:
        A list storing the total (summed) weight of the tree and the number of
        leaves in the tree.

    === Representation invariants ===
    - self.weight >= 0

    - (EMPTY TREE):
        If self.weight == 0, then self.value == [] and self.subtrees == [].
        This represents an empty simple prefix tree.
    - (LEAF):
        If self.subtrees == [] and self.weight > 0, this tree is a leaf.
        (self.value is a value that was inserted into this tree.)
    - (NON-EMPTY, NON-LEAF):
        If len(self.subtrees) > 0, then self.value is a list (*common prefix*),
        and self.weight > 0 (*aggregate weight*).

    - **NEW**
      This tree does not contain any compressible internal values.
      (See the assignment handout for a definition of "compressible".)

    - self.subtrees does not contain any empty prefix trees.
    - self.subtrees is *sorted* in non-increasing order of their weights.
      (You can break ties any way you like.)
      Note that this applies to both leaves and non-leaf subtrees:
      both can appear in the same self.subtrees list, and both have a `weight`
      attribute.
    """
    value: Any
    weight: float
    subtrees: List[CompressedPrefixTree]
    _weight_type: str
    _weight_data: [float, int]

    def __init__(self, weight_type: str) -> None:
        """Initialize an empty simple prefix tree.

        Precondition: weight_type == 'sum' or weight_type == 'average'.

        The given <weight_type> value specifies how the aggregate weight
        of non-leaf trees should be calculated (see the assignment handout
        for details).
        """
        self.value = []
        self.weight = 0.0
        self.subtrees = []
        self._weight_type = weight_type
        self._weight_data = [0.0, 0]

    def is_empty(self) -> bool:
        """Return whether this compressed prefix tree is empty."""
        return self.weight == 0.0

    def is_leaf(self) -> bool:
        """Return whether this compressed prefix tree is a leaf."""
        return self.weight > 0 and self.subtrees == []

    def __str__(self) -> str:
        """Return a string representation of this tree.

        You may find this method helpful for debugging.
        """
        return self._str_indented()

    def _str_indented(self, depth: int = 0) -> str:
        """Return an indented string representation of this tree.

        The indentation level is specified by the <depth> parameter.
        """
        if self.is_empty():
            return ''
        else:
            s = '  ' * depth + f'{self.value} ({self.weight})\n'
            for subtree in self.subtrees:
                s += subtree._str_indented(depth + 1)
            return s

    def __len__(self) -> int:
        """Return the number of values stored in this Autocompleter."""

        return self._weight_data[1]

    def insert(self, value: Any, weight: float, prefix: List) -> None:
        """Insert the given value into this Autocompleter.

        The value is inserted with the given weight, and is associated with
        the prefix sequence <prefix>.

        If the value has already been inserted into this prefix tree
        (compare values using ==), then the given weight should be *added* to
        the existing weight of this value.

        Preconditions:
            weight > 0
            The given value is either:
                1) not in this Autocompleter
                2) was previously inserted with the SAME prefix sequence
        """

        if self.is_empty():
            self.value = prefix
        else:
            new_val = []
            for a, b in zip(prefix, self.value):
                if a == b:
                    new_val.append(a)
                else:
                    break
            if len(new_val) < len(self.value):
                n = CompressedPrefixTree(self._weight_type)
                n.value, n.weight, n._weight_data, n.subtrees = \
                    self.value, self.weight, self._weight_data[:], self.subtrees
                self.value = new_val
                self.subtrees = [n]
        self._insert_helper(value, weight, prefix)

    def _insert_helper(self, value: Any, weight: float, prefix: List) -> None:
        if self.value == prefix:
            if any(sub.is_leaf() for sub in self.subtrees):
                inserted = False
                for subtree in self.subtrees:
                    if subtree.value == value and subtree.is_leaf():
                        subtree._weight_data[0] += weight
                        subtree.weight += weight
                        self._weight_data[0] += weight
                        self._adjust_weight()
                        self._sort_subtrees()
                        inserted = True
                        break

                if not inserted:
                    self._insert_leaf(value, weight)
            else:
                self._insert_leaf(value, weight)

        elif any(sub.value == prefix[:len(sub.value)]
                 and not sub.is_leaf() for sub in self.subtrees):
            i = 0
            while self.subtrees[i].value != prefix[:len(
                    self.subtrees[i].value)] or self.subtrees[i].is_leaf():
                i += 1
            subtree = self.subtrees[i]
            old_sub_length = len(subtree)
            subtree._insert_helper(value, weight, prefix)
            new_sub_length = len(subtree)
            self._weight_data[0] += weight
            self._weight_data[1] += new_sub_length - old_sub_length
            self._adjust_weight()
            self._sort_subtrees()
        else:
            new_subtree = CompressedPrefixTree(self._weight_type)
            inserted = False
            sub_p_list = [prefix[:k] for k in
                          range(len(prefix), len(self.value), -1)]
            for sub_p in sub_p_list:
                if inserted:
                    break
                for subtree in self.subtrees:
                    if subtree.value[:len(sub_p)] == sub_p \
                            and not subtree.is_leaf():
                        new_subtree.value = sub_p
                        new_subtree._weight_data[0] = subtree._weight_data[0]
                        new_subtree._weight_data[1] = subtree._weight_data[1]
                        new_subtree._adjust_weight()
                        new_subtree.subtrees.append(self.subtrees.pop(
                            self.subtrees.index(subtree)))
                        old_sub_length = len(subtree)
                        new_subtree._insert_helper(value, weight, prefix)
                        inserted = True
            if not inserted:
                new_subtree.value = prefix
                old_sub_length = 0
                new_subtree._insert_helper(value, weight, prefix)
            new_sub_length = len(new_subtree)
            self._weight_data[0] += weight
            self._weight_data[1] += new_sub_length - old_sub_length
            self._add_to_subtrees(new_subtree)
            self._adjust_weight()

    def _insert_leaf(self, value: Any, weight: float) -> None:
        """ Insert a leaf into an internal node that matches it's prefix.
        """
        subtree = CompressedPrefixTree(self._weight_type)
        subtree.value = value
        subtree.weight = weight
        subtree._weight_data = [float(weight), 1]
        self._weight_data[0] += weight
        self._weight_data[1] += 1
        self._adjust_weight()
        self._add_to_subtrees(subtree)

    def _add_to_subtrees(self, subtree: CompressedPrefixTree) -> None:
        """ A helper for the insert method that adds a subtree to self.subtrees.
        """
        if self.is_empty():
            self.subtrees = [subtree]
        else:
            inserted = False
            for i in range(len(self.subtrees)):
                if subtree.weight > self.subtrees[i].weight:
                    self.subtrees.insert(i, subtree)
                    inserted = True
                    break
            if not inserted:
                self.subtrees.append(subtree)

    def _adjust_weight(self) -> None:
        """ A helper for the insert method that adjusts the weight based on a
        given weight type."""
        if self._weight_type == "sum":
            self.weight = self._weight_data[0]
        elif self._weight_type == "average":
            self.weight = self._weight_data[0] / float(self._weight_data[1]) \
                if self._weight_data[0] != 0 else 0
        else:
            raise ValueError

    def _sort_subtrees(self) -> None:
        """ A helper for the insert method that sorts the subtrees in
        self.subtrees. """
        storage = [sub for sub in self.subtrees]
        self.subtrees = []
        for subtree in storage:
            self._add_to_subtrees(subtree)

    def autocomplete(self, prefix: List,
                     limit: Optional[int] = None) -> List[Tuple[Any, float]]:
        """Return up to <limit> matches for the given prefix.

        The return value is a list of tuples (value, weight), and must be
        ordered in non-increasing weight. (You can decide how to break ties.)

        If limit is None, return *every* match for the given prefix.

        Precondition: limit is None or limit > 0.
        """
        if self.is_empty():
            return []
        elif prefix in ([], self.value):
            return self._autocomplete_helper(limit)

        ac_big = []
        for subtree in self.subtrees:
            if limit is not None and len(ac_big) == limit:
                break
            if subtree.value == prefix[:len(subtree.value)]:
                temp = subtree.autocomplete(prefix, limit - len(ac_big)) \
                    if limit is not None else subtree.autocomplete(prefix)
                for val in temp:
                    _acc_insert(ac_big, val)
            elif len(subtree.value) > len(prefix):
                if subtree.value[:len(prefix)] == prefix:
                    temp = subtree._autocomplete_helper(
                        limit - len(ac_big)) if limit is not None else \
                        subtree._autocomplete_helper()
                    for val in temp:
                        _acc_insert(ac_big, val)
        return ac_big

    def _autocomplete_helper(self, limit: Optional[int] = None) \
            -> List[Tuple[Any, float]]:
        """ A helper for the autocomplete method that returns an autocompleted
        list with a given limit.
        """
        if self.is_leaf():
            return [(self.value, self.weight)]
        else:
            accumulator = []
            for subtree in self.subtrees:
                if len(accumulator) == limit:
                    break
                ac_subtree = subtree._autocomplete_helper(limit)
                for value in ac_subtree:
                    if limit is None or len(accumulator) < limit:
                        _acc_insert(accumulator, value)
                    else:
                        break
            return accumulator

    def remove(self, prefix: List) -> None:
        """Remove all values that match the given prefix.
        """
        if not prefix:
            self.subtrees = []
            self.weight = 0.0
            self._weight_data = [0.0, 0]
        else:
            self._remove_helper(prefix)
            self._sort_subtrees()

        if len(self.subtrees) == 1:
            subtree = self.subtrees[0]
            if subtree.is_leaf():
                self.value, self.weight, self._weight_data = \
                    None, 0.0, [0.0, 0]
            else:
                self.value, self.weight, self._weight_data = \
                    subtree.value, subtree.weight, subtree._weight_data
            self.subtrees = subtree.subtrees[:]

    def _remove_helper(self, prefix: List) -> Optional[(float, int)]:
        """
        A helper for the remove method that returns an optional tuple that keeps
        track of change in total weight and change in total number of leaves.
        """
        if self.value == prefix:
            self.weight = 0.0
            return self._weight_data[0], self._weight_data[1]
        else:
            total_big = 0
            count_big = 0
            for subtree in reversed(self.subtrees):
                if subtree.value == prefix[:len(subtree.value)]:
                    total, count = subtree._remove_helper(prefix)
                    self._weight_data[0] -= total
                    self._weight_data[1] -= count
                    total_big += total
                    count_big += count
                    if subtree.is_empty():
                        self.subtrees.remove(subtree)
                    self._adjust_weight()
                elif len(subtree.value) > len(prefix):
                    if subtree.value[:len(prefix)] == prefix:
                        total, count = subtree._weight_data[0], \
                                       subtree._weight_data[1]
                        subtree.weight = 0.0
                        self._weight_data[0] -= total
                        self._weight_data[1] -= count
                        total_big += total
                        count_big += count
                        self.subtrees.remove(subtree)
                        self._adjust_weight()
            for i in range(len(self.subtrees)):
                subtree = self.subtrees[i]
                if len(subtree.subtrees) == 1:
                    if not subtree.subtrees[0].is_leaf():
                        self.subtrees[i] = subtree.subtrees[0]
            self._sort_subtrees()
            return total_big, count_big


def _acc_insert(arr: List[Tuple[Any, float]], val: Tuple[Any, float]) -> None:
    """
    Helper method for the autocomplete method which mutates a list to include
    a value.
    """
    if arr == [] or val[1] <= arr[-1][1]:
        arr.append(val)
    else:
        index = 0
        while val[1] < arr[index][1] and index < len(arr):
            index += 1
        arr.insert(index, val)


